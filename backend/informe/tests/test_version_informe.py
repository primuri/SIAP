...
from ..models import VersionInforme
from django.contrib.auth.models import Group
from usuario_personalizado.models import Usuario
from django.core.files.uploadedfile import SimpleUploadedFile

class VersionInformeTests(APITestCase):
    ...
    def test_post_version_informe(self):

        url = reverse('versioninforme-list')
        response = self.client.post(url, self.data, format='json')

        # Verificaciones
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(VersionInforme.objects.count(), 1)
        self.assertEqual(VersionInforme.objects.get().numero_version, '1')

    def test_put_version_informe(self):
        update_data = {
            'numero_version': '2',
            'fecha_presentacion': '2024-01-30T00:00:00Z',
            'id_informe_fk': self.informe_id,
            'id_evaluacion_cc_fk': None,
            'id_oficio_fk': self.oficio_id,
            'id_documento_informe_fk': self.documento_id
        ...
        self.client.post(reverse('versioninforme-list'), self.data, format='json')
        url = reverse('versioninforme-detail', args=['1'])
        response = self.client.put(url, update_data, format='json')

        # Verificaciones
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(VersionInforme.objects.get().numero_version, '2')

    def test_get_lista_version_informes(self):
        data2 = {
            'numero_version': '3',
            'fecha_presentacion': '2023-01-30T00:00:00Z',
            'id_informe_fk': self.informe_id,
        ...
        self.client.post(reverse('versioninforme-list'), self.data, format='json')
        self.client.post(reverse('versioninforme-list'), data2, format='json')
        url = reverse('versioninforme-list')
        response = self.client.get(url)
        url2 = reverse('versioninforme-detail', args=['2'])
        ...
        self.assertEqual(response2.data['numero_version'], '3')
        
    def test_get_buscar_version_informe(self):
        ...
        # Verificaciones
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['numero_version'], '1')

    def test_delete_version_informe(self):
        ...
        self.assertEqual(VersionInforme.objects.count(), 0)