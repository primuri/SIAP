...
class ProductoTests(APITestCase):
    ...
    def test_post_producto(self):
        ...
        self.assertEqual(Producto.objects.get().detalle, 'Producto asociado')

    def test_put_producto(self):
        update_data = {
            'id_version_proyecto_fk': self.version_id,
            'fecha': '2023-05-30T00:00:00Z',
            'detalle': 'Producto asociado editado'
        }
        
        self.client.post(reverse('producto-list'), self.data, format='json')
        url = reverse('producto-detail', args=['1'])
        response = self.client.put(url, update_data, format='json')

        # Verificaciones
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Producto.objects.get().detalle, 'Producto asociado editado')

    def test_get_lista_productos(self):
        data2 = {
            'id_version_proyecto_fk': self.version_id,
            'fecha': '2024-10-30T00:00:00Z',
            'detalle': 'Producto 2 asociado editado'
        }

        self.client.post(reverse('producto-list'), self.data, format='json')
        self.client.post(reverse('producto-list'), data2, format='json')
        url = reverse('producto-list')
        response = self.client.get(url)
        url2 = reverse('producto-detail', args=['2'])
        ...
        self.assertEqual(response2.data['fecha'], '2024-10-30T00:00:00Z')

        
    def test_get_buscar_producto(self):
        ...
        # Verificaciones
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['detalle'], 'Producto asociado')
