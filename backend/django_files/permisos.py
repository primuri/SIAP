...
class PermisoPorRol(permissions.BasePermission):
    def has_permission(self, request, view):
        if request.user.groups.filter(name='evaluador').exists() and request.user.groups.filter(name='investigador').exists():
            return (request.method == 'GET' or (request.method in ['GET', 'POST', 'PATCH'] and view.view_name in ['evaluaciones', 'respuestas_evaluaciones']))
        else: 
