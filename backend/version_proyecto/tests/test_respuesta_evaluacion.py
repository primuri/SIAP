...
class RespuestaEvaluacionTests(APITestCase):
    ...
    def setUp(self):
        ...
        response =self.client.post(reverse('evaluacion-list'), self.evaluacion, format='json')
        self.evaluacion_id = response.data['id_evaluacion']

        self.data = {
            'id_evaluacion_fk' : self.evaluacion_id,
    ...
    def test_post_respuesta_evaluacion(self):
        ...
        self.assertEqual(RespuestaEvaluacion.objects.count(), 1)
        self.assertEqual(RespuestaEvaluacion.objects.get().pregunta, '¿Que hora es?')

    def test_put_respuesta_evaluacion(self):
        update_data = {
            'id_evaluacion_fk' : self.evaluacion_id,
            'pregunta': '¿Como eres?',
            'respuesta': 'Malo'
        ...
        self.client.post(reverse('respuestaevaluacion-list'), self.data, format='json')
        url = reverse('respuestaevaluacion-detail', args=['1'])
    ...
    def test_get_lista_respuesta_evaluaciones(self):
        ...
        self.client.post(reverse('respuestaevaluacion-list'), self.data, format='json')
        self.client.post(reverse('respuestaevaluacion-list'), data2, format='json')
        url = reverse('respuestaevaluacion-list')
        response = self.client.get(url)
        url2 = reverse('respuestaevaluacion-detail', args=['2'])
        ...
        self.assertEqual(response2.data['respuesta'], 'Brandon')

        
    def test_get_buscar_respuesta_evaluacion(self):
        ...
        # Verificaciones
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['respuesta'], '7:30')

    def test_delete_respuesta_evaluacion(self):
